...
const findRoute = (fromPt, toPt, isPointObstacle, opt) => {
  ...
  let sourceAnchor = new Point(fromPt.x, fromPt.y, 0, 0, fromPt.o);
  let targetAnchor = new Point(toPt.x, toPt.y, 0, 0, toPt.o);
  ...
  sourceAnchor.type = 'start';
  startPoints = [sourceAnchor];
  endPoints = [targetAnchor];
  ...
  if (startPoints.length > 0 && endPoints.length > 0) {
    ...
    while (!openSet.isEmpty() && loopsRemaining > 0) {
      ...
      let _startKey = sourceAnchor.x + '@' + sourceAnchor.y;
      let _endKey = targetAnchor.x + '@' + targetAnchor.y;
      let _directions = [];

      if (_startKey === currentKey) {
        _directions.push(directions[sourceAnchor.o]);
      } else if (_endKey === currentKey) {
        _directions.push(directions[targetAnchor.o]);
      } else {
        for (let i = 0; i < numDirections; i++) {
          _directions.push(directions[i]);
        }
      }
      ...
      _directions.forEach((direction) => {
        ...
        if (!(isPathBeginning && isStart) && directionChange > opt.maxAllowedDirectionChange) return;
        ...
        if (openSet.isClose(neighborKey) || isPointObstacle(neighborPoint)) return;
        ...
        if (endPointsKeys.indexOf(neighborKey) >= 0) { // neighbor is an end point
          ...
          if (!isNeighborEnd) {
            ...
            if (endDirectionChange > opt.maxAllowedDirectionChange) return;
          }
        }
...
function drawAdvancedManhattan(sourcePoint, targetPoint, options) {
  ...
  const fromPt = {
    ...
    nw: sourcePoint.nodeSize[0] || 145,
    nh: sourcePoint.nodeSize[1] || 50,
    o: _dirMap[(sourcePoint.orientation || '').toString()]
  };
  const toPt = {
    x: targetPoint.pos[0],
    y: targetPoint.pos[1],
    ...
    nw: targetPoint.nodeSize[0] || 145,
    nh: targetPoint.nodeSize[1] || 50,
    o: _dirMap[(targetPoint.orientation || '').toString()]
  };
